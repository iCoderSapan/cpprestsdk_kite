...
namespace KiteConnect
{
    class Utils
    {
        ...
        /// <param name="Json">Json string to deserialize.</param>
        /// <returns>Json in the form of nested string dictionary.</returns>
        static vector<string>& JsonArrayDeserialize(json::array jsonArray, vector<string> &result)
        {
            for(auto itr = jsonArray.being(); itr != jsonArray.end(); itr++)
            {
        ...
        static inline void AddIfNotNull(map<string, string> &params, string &Key, string &value)
        {
            if (!value.is_empty())
                params[Key] = value;
        }
