...
{ 
   ...
   { 
         *fileStream = outFile;
         ...
         pplx::task<http_response> resp = client.request(methods::POST, utility::conversions::to_utf8string(builder.to_string()), req_string,
   ...
   { 
         ...
         std::cout<<"Contect-type: "<<response.headers().content_type()<<std::endl;
...
std::string api_key = utility::conversions::to_utf8string("utmcr52rmqeze8sc");
std::string api_secret = utility::conversions::to_utf8string("443isdhujy5d45jw85b4ldr0wmbg07me");
std::string request_token= utility::conversions::to_utf8string("xxx");
http_client client(U("https://api.kite.trade")); 
...
builder.append_query(U("api_secret"), api_secret);
builder.append_query(U("request_token"), request_token);
std::string url_data(builder.to_string());
std::cout<<"+++++++++++:"<<url_data.erase(0,2);
